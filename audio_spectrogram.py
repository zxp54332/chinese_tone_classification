...
def main():
    ...
    def train_transforms(batch):
        ...
        # return output_batch
        sample = batch["path"]
        ast_features = feature_extractor(sample["array"], sampling_rate=feature_extractor.sampling_rate)
        array = ast_features["input_values"][0]
    ...
    def compute_metrics(eval_pred):
        ...
        acc = acc_metric.compute(predictions=predictions, references=eval_pred.label_ids)["accuracy"]
    ...
    if training_args.do_train:
        ...
        raw_datasets["train"] = raw_datasets["train"].map(train_transforms)

    if training_args.do_eval:
        if data_args.max_eval_samples is not None:
            raw_datasets["eval"] = raw_datasets["eval"].shuffle(seed=training_args.seed)
        # Set the validation transforms
        raw_datasets["eval"] = raw_datasets["eval"].map(train_transforms)
